          ...
          {(['applications', 'messages'] as const).map((t_) => (
            <button
              ...
              className={`px-5 py-2 rounded-lg text-xs font-semibold transition-all ${
                tab === t_ ? 'bg-white text-sdy-black shadow-sm' : 'text-gray-400 hover:text-gray-600'
              }`}
            >
              {t_ === 'applications'
          ...
          {loading ? (
              ...
              <table className="w-full text-left">
                <thead>
                  <tr className="border-b border-gray-100">
                    ...
                    <th className="px-5 py-3.5 text-xs font-semibold text-gray-400 uppercase tracking-wider">{t({ mn: 'Байршил / Нас', en: 'Location / Age' })}</th>
                ...
                <tbody className="divide-y divide-gray-50">
                  {applications.length === 0 ? (
                    <tr><td colSpan={6} className="px-5 py-16 text-center">
                      ...
                      <p className="text-sm font-medium text-gray-400">{t({ mn: 'Өргөдөл байхгүй байна', en: 'No applications yet' })}</p>
                    </td></tr>
                  ) : applications.map((app) => (
                    <motion.tr key={app.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="hover:bg-gray-50/60 transition-colors">
                      <td className="px-5 py-3.5">
                        <div className="font-semibold text-sdy-black text-sm">{app.last_name} {app.first_name}</div>
                      </td>
                      <td className="px-5 py-3.5">
                      ...
                      <td className="px-5 py-3.5">
                        ...
                        <div className="text-xs text-gray-400">{t({ mn: `${app.age} нас`, en: `Age ${app.age}` })}</div>
                      </td>
                      <td className="px-5 py-3.5">
                      ...
                      <td className="px-5 py-3.5">
                        <div className="flex items-center justify-end gap-1.5">
                          ...
                          <button onClick={() => deleteApp(app.id)} className="p-2 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-lg transition-all" title="Delete"><Trash2 size={15} /></button>
                        </div>
                      </td>
                    </motion.tr>
                  ))}
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                <thead>
                  <tr className="border-b border-gray-100">
                    <th className="px-5 py-3.5 text-xs font-semibold text-gray-400 uppercase tracking-wider">{t({ mn: 'Илгээгч', en: 'From' })}</th>
                    <th className="px-5 py-3.5 text-xs font-semibold text-gray-400 uppercase tracking-wider">{t({ mn: 'Гарчиг', en: 'Subject' })}</th>
                    <th className="px-5 py-3.5 text-xs font-semibold text-gray-400 uppercase tracking-wider">{t({ mn: 'Зурвас', en: 'Message' })}</th>
                    <th className="px-5 py-3.5 text-xs font-semibold text-gray-400 uppercase tracking-wider">{t({ mn: 'Огноо', en: 'Date' })}</th>
                    <th className="px-5 py-3.5 text-xs font-semibold text-gray-400 uppercase tracking-wider text-right">{t({ mn: 'Үйлдэл', en: 'Action' })}</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-50">
                  {messages.length === 0 ? (
                    <tr><td colSpan={5} className="px-5 py-16 text-center">
                      <Mail size={24} className="text-gray-200 mx-auto mb-3" />
                      <p className="text-sm font-medium text-gray-400">{t({ mn: 'Зурвас байхгүй байна', en: 'No messages yet' })}</p>
                    </td></tr>
                  ) : messages.map((msg) => (
                    <motion.tr key={msg.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="hover:bg-gray-50/60 transition-colors">
                      <td className="px-5 py-3.5">
                        <div className="font-semibold text-sdy-black text-sm">{msg.name}</div>
                        <div className="text-xs text-gray-400">{msg.email}</div>
                      </td>
                      <td className="px-5 py-3.5">
                        <div className="text-sm text-gray-600 max-w-[180px] truncate">{msg.subject}</div>
                      </td>
                      <td className="px-5 py-3.5">
                        <div className="text-sm text-gray-500 max-w-[280px] line-clamp-2 leading-relaxed">{msg.message}</div>
                      </td>
                      <td className="px-5 py-3.5">
                        <div className="text-xs text-gray-400 whitespace-nowrap">{new Date(msg.created_at).toLocaleDateString()}</div>
                      </td>
